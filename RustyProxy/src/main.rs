...
async fn handle_client(mut client_stream: TcpStream, addr: SocketAddr) -> Result<(), Error> {
    let mut buffer = [0; 4096]; // Buffer maior para requisições HTTP
    ...
    let dest_addr = match (method.as_str(), host, port) {
        ...
        _ => {
            eprintln!("[{}] Requisição inválida ou sem host", addr);
            client_stream
    ...
    let server_stream = match TcpStream::connect(&dest_addr).await {
        Ok(stream) => stream,
        Err(e) => {
            eprintln!("[{}] Erro ao conectar-se ao servidor {}: {}", addr, dest_addr, e);
            client_stream
